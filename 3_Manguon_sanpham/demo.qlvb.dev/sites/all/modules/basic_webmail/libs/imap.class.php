  ...
  */
  function search($query, $criteria, $flags) {
	...
	*/
	...
	{
		if(!$this->mbox) return FALSE;
    	$folders = imap_list($this->mbox, '{' . $this->host .':' . $this->port . '}', $filter);
    	return $folders;
	}
	...
	*/
  	function get_overview()
  	{
		...
		if ($check -> Nmsgs > 0) {
			return imap_fetch_overview($this->mbox, "1:".$check -> Nmsgs."", 0);
		} 
		return false;
  	}
	/*
	@des returns an array of objects with headers for all messages in the folder
	@author : truongnx.ithut@gmail.com
	*/
  	function get_overview_uid($uid)
  	{
  		if(!$this->mbox) return FALSE;
  		$check = imap_check($this->mbox);
		if ($check -> Nmsgs > 0) {
		 	return imap_fetch_overview($this->mbox, $uid.":*", FT_UID);
		}
		return FALSE;
  	}
    /*
    ...
    */
	...
	function move($mid, $destination, $options=CP_UID ) {
		if(!$this->mbox) return FALSE;
		return imap_mail_move($this->mbox, $mid, $destination, $options);
	}
  	function delete($mid,$options=FT_UID ) {
		if (!$this->mbox) return FALSE;
  		return imap_delete($this->mbox, $mid, $options);
  	}

    /*
    returns the structure of the IMAP message in form of an object
    */
    function get_structure($mid, $options=0) {
 		if(!$this->mbox) return FALSE;
    ...
    */
    function get_body($mid, $part, $options=0) {
       	if(!$this->mbox) return FALSE;
	...
	/*
		...
		06/03/2012
	*/
  	function get_message_overview($mid)
  	{
  		if(!$this->mbox) 
  			return FALSE;  	
		$overview=imap_fetch_overview($this->mbox,$mid, 0);
		return $overview[0];
    ...
    */
    function set_flag($mid,$flags,$option=ST_UID ) {
       	if(!$this->mbox) return FALSE;
       	if(empty($flags)) return FALSE;

       	return imap_setflag_full($this->mbox, $mid, $flags, $option);
    }

    /*
    clears a flag on a message
    */
    function clear_flag($mid,$flags,$option=ST_UID) {
       	if(!$this->mbox) return FALSE;
       	if(empty($flags)) return FALSE;

       	return imap_clearflag_full($this->mbox, $mid, $flags, $option);
    }
